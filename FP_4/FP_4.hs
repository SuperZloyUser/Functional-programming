{- 
Задание:
А) Определить функции высших порядков для выполнения тех или иных запросов.

Б) Определить функции для добавления, удаления и "модификации" элементов списков.

В) Определить функции, демонстрирующие корректное выполнение запросов и операций из п.Б.
...
data Department = Department {
  name :: String,
  countOfTeachers :: Integer,
  countOfProfessors :: Integer
} deriving (Show)
...
createDepartment pName pCountOfTeachers pCountOfProfessors
  ...
  | otherwise = department where
        department = Department {
        name = pName,
        countOfTeachers = pCountOfTeachers,
        countOfProfessors = pCountOfProfessors }

-- ***Функця, добавляющая в список структур ещё одну
addDepartment :: Department -> [Department] -> [Department]
